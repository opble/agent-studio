  ...
  return (
    <div className="flex items-center gap-3 border-b border-[var(--color-border)] bg-[var(--color-surface-raised)] px-4 py-3 md:px-5">
      ...
      <div className="min-w-0 flex-1">
        <p className="text-sm font-semibold leading-tight text-[var(--color-text-primary)]">{agent.name}</p>
        {agent.description && (
          <p className="mt-0.5 truncate text-[11px] leading-none text-[var(--color-text-muted)]">{agent.description}</p>
        )}
      </div>
      {agent.model && (
